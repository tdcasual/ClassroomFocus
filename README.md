...
通过摄像头分析学生专注状态（眨眼/低头/疑似睡眠），同时录制整节课音频；课后将音频转写为文本并用 OpenAI 兼容大模型整理课程知识点，再把“睡觉时间段”与“讲课内容/知识点”对齐，输出可视化报告。
...
- 报告生成：整节课音频 ASR → `transcript.txt` → LLM 总结/时间线 → 将睡觉区间与讲课主题/ASR 文本关联。
...
3) Generate Report（离线 ASR + LLM 总结 + 睡觉区间关联讲课内容）
...
`stats.json` 中会包含每个 track 的睡觉区间、区间内的 ASR 文本片段，以及与课程主题（`lesson_summary.timeline`）的关联结果。
...
- `tools/web_viz_server.py` – FastAPI Web UI + 报告生成链路（ASR + LLM 总结 + 睡觉区间关联）。
